import { parseParamCount } from './formatters'
import {
  getCatalogValidationErrorMessage,
  ModelDatabaseEntrySchema,
} from './schemas'
...
export const estimateRamRequirements = (parameterCount: string): Record<string, number> => {
  const paramsB = parseParamCount(parameterCount)
  if (!paramsB) return {}
...
export const normalizeModelEntry = (entry: ModelDatabaseEntry): ModelDatabaseEntry => {
  ...
  const activeParams =
    typeof entry.active_params_b === 'number' && Number.isFinite(entry.active_params_b)
      ...
      : parseParamCount(entry.parameter_count)
...
export const loadModelDatabase = async (): Promise<ModelDatabaseEntry[]> => {
  ...
  if (!response.ok) {
    throw new Error(`Model catalog could not be loaded (HTTP ${response.status}).`)
  }
  let data: unknown
  try {
    data = await response.json()
  } catch {
    throw new Error('Model catalog is not valid JSON.')
  }
  const result = ModelDatabaseEntrySchema.array().safeParse(data)
  if (!result.success) {
    throw new Error(getCatalogValidationErrorMessage(result.error.issues))
  }
